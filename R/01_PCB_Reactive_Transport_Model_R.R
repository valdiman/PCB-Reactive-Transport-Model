...
#Set working directory - replace "YourWdHere" with the file path containing data files
setwd("YourWdHere")

#Read data from .csv - file shown is from Iowa Research Online (IRO) data deposit Bako et. al. 2022 
exp.data <- read.csv("Data/08_Dataset_FinalPCBMass_2021-12-06.csv", fileEncoding="UTF-8-BOM")

#Defining control group
control <- c("Ctrl", "Sap")

#Pull congener-specific data from the dataset & calculate mean values for each sampler-treatment combination at each timepoint
...
pcb.52 <- left_join(exp.mf.control, exp.mpuf.control)  %>% 
  ...
  add_row(time = 0, "PCB 52 mass in SPME (ng/cm); Control" = 0, "PCB 52 mass in PUF (ng); Control" = 0, 
          "PCB 52 mass in SPME (ng/cm); LB400" = 0, "PCB 52 mass in PUF (ng); LB400" = 0,
          "PCB 52 mass in SPME (ng/cm); LB400+Sap" = 0, "PCB 52 mass in PUF (ng); LB400+Sap" = 0, .before = 1)
...
parms <- c(ka = 1953, kd = 3.2) # Input second guess for parameters
# fitting
